...
export function NeededMachinesGraph() {
  const neededMachinesHistory = useSimulationStore((state) => state.neededMachinesHistory);

  const width = 800;
  const height = 200;
  const padding = 40;

  const maxNeeded = Math.max(...neededMachinesHistory, 10); // Ensure a minimum y-axis scale

  const xScale = (index: number) => {
    if (neededMachinesHistory.length <= 1) return padding;
    return padding + (index / (neededMachinesHistory.length - 1)) * (width - 2 * padding);
  };

  const yScale = (value: number) => {
    return height - padding - (value / maxNeeded) * (height - 2 * padding);
  };

  const pathData = neededMachinesHistory
    .map((value, index) => {
  ...
  return (
    <Paper elevation={3} sx={{ p: 2, mt: 2, width: '100%' }}>
      <Typography variant="h6" gutterBottom>Additional Hosts Needed</Typography>
      <Box sx={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
        <svg width="100%" height={height} viewBox={`0 0 ${width} ${height}`}>
          {/* Y-axis labels */}
          {[...Array(5)].map((_, i) => {
            const value = Math.round((i / 4) * maxNeeded);
            return (
              <g key={value}>
          ...
          <g>
            ...
            <text x={width - padding} y={height - padding + 20} textAnchor="end" fill="#888" fontSize="12">
              {neededMachinesHistory.length - 1}
            </text>
